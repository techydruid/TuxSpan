...
4. Open TuxSpan and go to **Settings > Companion apps**. Confirm that the required apps are detected.
5. Grant TuxSpan **Run commands in Termux environment** when prompted.
6. In TuxSpan, copy the one-time Termux consent command. Open Termux, paste it, and press Enter:

   ```sh
   mkdir -p ~/.termux && printf '\nallow-external-apps=true\n' >> ~/.termux/termux.properties && termux-reload-settings
   ```

7. Return to TuxSpan, choose **Canvas**, **Studio**, or **Spark**, and open its review screen.
8. Read the generated setup recipe, then tap **Run reviewed setup**. TuxSpan opens Termux and shows progress while the packages and Linux image are installed.
9. Keep Termux open until the command finishes. Do not close Termux, force-stop either app, or change networks during the download.
10. Return to TuxSpan and tap **Verify**. When the workspace is ready, launch it from **Home**.
